...
func TestSession_InitSession(t *testing.T) {
	...
	for rv.Kind() == reflect.Ptr || rv.Kind() == reflect.Interface {
		...
		rv = rv.Elem()
		fmt.Println("rvElem:", rv)
	}

	if !rv.IsValid() {
		t.Error("invalid type or kind; king:", rv.Kind(), "type", rv.Type())
	}

	if sessKind != reflect.ValueOf(sm).Kind() {
		t.Error("wrong king returned testing cookie session. Expected", reflect.ValueOf(sm).Kind(), "and got", sessKind)
	}
